    ...
    def _setup_routes(self):
        ...
        def create_user():
            ...
            try:
                ...
                user_id = self.db.execute_query(
                    """INSERT INTO users (telegram_id, main_wallet_address, metadata) 
                       VALUES (?, ?, ?)""",
                    (telegram_id, wallet_address, json.dumps({'created_via': 'api'}))
                )
                
                # Store address variants
                for variant_type, variant_address in address_variants.items():
                    self.db.execute_query(
                        """INSERT OR IGNORE INTO user_address_variants 
                           (user_id, address_variant, variant_type) VALUES (?, ?, ?)""",
                        (user_id, variant_address, variant_type)
                    )
        ...
        def get_balance_by_wallet(wallet_address: str):
            ...
            try:
                ...
                # Find user by any address variant
                user_data = self.db.execute_query(
                    """SELECT DISTINCT u.id, u.telegram_id, u.main_wallet_address,
                              ub.balance, ub.available_balance, ub.locked_balance,
                              u.total_deposited, u.transaction_count
                       FROM users u
                ...
                return jsonify({
                    ...
                    'total_deposited': float(user_data['total_deposited'] or 0),
                    'transaction_count': user_data['transaction_count'] or 0,
                    'correlation_id': g.correlation_id
                }), 200
